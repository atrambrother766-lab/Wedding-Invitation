...
$('#adminForm').addEventListener('submit', async event => {
  ...
  try {
    ...
    $('#loginError').textContent = 'Email किंवा पासवर्ड चुकीचा आहे.';
    $('#loginError').classList.remove('hidden');
  }
});
$('#adminLogout').addEventListener('click', async () => {
  try { await firebase.auth().signOut(); } catch (error) { console.warn('Firebase logout failed:', error); }
  setAdminMode(false);
  $('#invitationScreen').classList.add('hidden');
  $('#welcomeScreen').classList.remove('hidden');
});
$('#editCeremonyBtn').addEventListener('click', () => showInvitation('', true)); $('#whatsappShareBtn').addEventListener('click', openShareModal); $('#homeLogoutBtn').addEventListener('click', async () => {
  try { await firebase.auth().signOut(); } catch (error) { console.warn('Firebase logout failed:', error); }
  setAdminMode(false);
});
